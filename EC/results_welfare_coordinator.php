    ...
    " role="navigation">
      <div class="container">
        ...
        <div id= "autoload" style="padding-top:20px;  " class="">
            ...
            <center><h2 class="specialHead"><b>Welfare Coordinator</b></h2></center>
          </div>
          ...
          <div class="col-sm-12">
            <?php
              ...
              if(mysqli_num_rows($result)>0)
              {
                ...
                //BJP
                $sql ="SELECT * FROM voters WHERE welfare_coordinator = 'Boandwoh Kwasi' ";
                $result= mysqli_query($link, $sql);
                ...
                if($query_num_rows > 0)
                {
                  ...
                  $percentage = $BK/$total*100;
                  $approximated_percentage = substr($percentage, 0, 5);
                  $bk_value= $approximated_percentage;

                  echo "<strong><span style='font-size: 20px;'> Boandwoh Kwasi </span>       <span style='float: right; font-size: 27px'>$approximated_percentage %<span></strong><br>";
                  echo "<span style ='font-size: 20px'>$BK Votes</span>  
                  <div class='progress'>
                    <div class='progress-bar progress-bar-success' role='progressbar' aria-valuenow=\"70\" aria-valuemin=\"0\" aria-valuemax=\"100\" style='width: ".$bk_value."%'>
                      <span class='sr-only'>Boandwoh Kwasi</span>
                    </div>
                  </div>
                  ";
                }
                ...
                // CONGRESS
                $sql ="SELECT * FROM voters WHERE welfare_coordinator = 'Winfred Quaye' ";
                $result= mysqli_query($link, $sql);
                ...
                if(mysqli_num_rows($result)>0)
                {
                  ...
                  <div class='progress'>
                    <div class='progress-bar progress-bar-primary' role='progressbar' aria-valuenow=\"70\" aria-valuemin=\"0\" aria-valuemax=\"100\" style='width: ".$wq_value."%'>
                      <span class='sr-only'>Winfred Quaye</span>
                    </div>
                  </div>
                  ";
                }
                ...
                // Ruth Aidoo
                $sql ="SELECT * FROM voters WHERE welfare_coordinator = 'Samuel Brown' ";
                $result= mysqli_query($link, $sql);
                ...
                if(mysqli_num_rows($result)>0)
                {
                  ...
                  $percentage = $SB/$total*100;
                  $approximated_percentage = substr($percentage, 0, 5);
                  $sb_value= $approximated_percentage;

                  echo "<strong><span style='font-size: 20px;'>   Samuel Brown </span>  <span style='float: right; font-size: 27px'>$approximated_percentage %<span></strong><br>";
                  echo "<span style ='font-size: 20px'>$SB Votes</span>   
                  <div class='progress'>
                    <div class='progress-bar progress-bar-info' role='progressbar' aria-valuenow=\"70\" aria-valuemin=\"0\" aria-valuemax=\"100\" style='width: ".$sb_value."%'>
                      <span class='sr-only'>Samuel Brown</span>
                    </div>
                  </div>
                  ";
                }
                         ...
                         // Skipped
                         $sql ="SELECT * FROM voters WHERE welfare_coordinator = 'Skipped' ";
                         $result= mysqli_query($link, $sql);
...
$(document).ready(function(){
    $('#autoload').load('results_welfare_coordinator.php') 
    refresh();
});

function refresh(){
    setTimeout( function(){
        $('#autoload').fadeOut('slow').load('results_welfare_coordinator.php').fadeIn('slow');
        refresh();
    }, 3000); 
}
</script>
